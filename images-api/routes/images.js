    ...
    for ( let index in req.files ) {
      ...
      await s3Bucket.putObject( data, ( err, data ) => {
        if ( err ) {
          ...
          console.log( 'Image was uploaded: ' + file.id )
        }
      } );
    }

    const savedImages = await knex( 'images' ).insert( savedFiles ).returning( '*' );
    const ids = savedImages.map( image => {
      return image.id
    } );
    res.status( 200 ).json( {
      success: true,
...
router.put( '/image', auth.isAuthenticated, async ( req, res ) => {
  try {
    ...
    await knex( 'images' ).where( 'id', req.body.id ).update( {
...
router.get( '/', auth.isAuthenticated, async ( req, res ) => {
  try {
    ...
    for ( let index in userImages ) {
      ...
      let url = await s3Bucket.getSignedUrl( 'getObject', {
        Key: image.id
      } );
      image.url = url;
    }
